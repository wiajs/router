...
(function (global, factory) {
   ...
   */ let Router = class Router {
     ...
     */ constructor(opts){
          ...
          // 缓存所有page实例
          this.ps = {};
          /** @type {string[]} */ this.ids = [] // 页面id
          ;
          // 缓存所有Page中的dom视图，不是$dom
          this.vs = {};
          this.vps = new Map() // dom page 映射，通过 dom对象查找page实例！
          ;
          ...
          ;
          /** @type {*} */ this.refresh = {};
          /** @type {*} */ this.page = null // 当前 page 实例
          ;
              ...
              if (this.opt.mode === 'local') {
                  ...
                  const pgHtml = new Promise((resHtml, rejHtml)=>{
                      ...
                      $.get(pgurl).then((rs)=>{
                          ...
                          ;
                          // 去掉 vite 添加的 脚本标签
                          p.html = this.vite ? rs.replace('<script type="module" src="/@vite/client"></script>', '') : rs;
                          p.param = param;
                          // 保存应用所有者和应用名称
                          p.owner = owner;
                          p.appName = name;
                  ...
                  const pgCss = new Promise((resCss, rejCss)=>{
                      ...
                      if (this.vite) {
                          import(`${this.opt.local}/page/${path}.css`).then((m)=>resCss(m)).catch((err)=>rejCss(err));
                      } else {
                          $.get(pgurl).then((rs)=>{
                              // debugger;
                  ...
                  if (this.opt.cos.includes('localhost:')) url = `${this.opt.cos}/page/${path}.js?v=${Date.now()}`;
     ...
     */ async createApp(owner, name) {
          ...
          try {
              ...
              // 需创建应用
              let appCls = null;
              if (_.opt.mode === 'local') // eslint-disable-next-line
                  ...
                  this.checkToken(owner, name, tk).then((rs)=>{
                      if (rs) {
                          ...
                          this.getCode(tk).then((code)=>{
                              if (code) {
                                  $.get(`${self.opt.api}/${owner}/${name}/${API.getToken}`, `code=${code}`).then((r)=>{
                                      if (r) {
          ...
          return new Promise((res, rej)=>{
              ...
              try {
                  if (!token) res(R);
                      ...
                      $.get(`${this.opt.api}/${owner}/${name}/${API.checkToken}`, `token=${token}`).then((rs)=>{
     ...
     */ getCode(token) {
          return new Promise((res, rej)=>{
              let R = '';
              try {
                  $.get(`${this.opt.api}/${API.getCode}`, `token=${token}`).then((rs)=>{
     ...
     */ to(p, refresh = false, lastHash = '') {
          ...
          // 切换应用
          this.switchApp(p.owner, p.appName, p.path).then((rt)=>{
              if (rt) {
                  // 记录当前page实例
