...
(function (global, factory) {
  ...
  (global = global || self, factory(global['@wiajs/Router'] = {}));
  ...
  function () {
    ...
    _proto.load = function load(url, param) {
      ...
      try {
        R = new Promise(function (res, rej) {
          ...
          if (!owner || !name || !page) res('');else {
            _this2.switchApp(owner, name, path).then(function (rs) {
              if (rs) {
                // 本地调试状态，直接获取本地页面
    ...
    _proto.switchApp = function switchApp(owner, name, path) {
      ...
      return new Promise(function (res, rej) {
        ...
        try {
          if (owner === _this3.owner && name === _this3.name) res(true);else {
          ...
          _this4.checkToken(owner, name, tk).then(function (rs) {
            if (rs) {
              ...
              _this4.getCode(tk).then(function (code) {
                if (code) {
                  $.get(that.opt.api + "/" + owner + "/" + name + "/" + API.getToken, "code=" + code).then(function (r) {
                    if (r) {
