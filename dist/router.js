   ...
   */ let Router = class Router {
              ...
              try {
                  ...
                  // appCls = _.pages?.['./src/index'] ?? __webpack_require__('./src/index.js')
                      ...
                      if (!$.M.m[m]) {
                          ...
                          // debugger;
                          // console.log(r);
                          if (r == null ? void 0 : r.js) $.M.add(r.js) // 模块加入到缓存数组
                          ;
                      }
     ...
     */ wxAuth(app, hash, param) {
          ...
          try {
              const k = 'wxAuthCnt';
              var _$_store_get;
              let cnt = (_$_store_get = $.store.get(k)) != null ? _$_store_get : 0;
              if (cnt === '') cnt = 0;
     ...
     */ getToken(app, sid, code, loginType, verifier) {
          ...
          return _async_to_generator(function*() {
              ...
              try {
                  ...
                  // 存在登录令牌，获取用户信息
                  if (token) {
                      const u = $._user ? $._user : yield _.getUser(app, token);
                      console.log({
                          u
                      }, 'getToken');
                      if (u) {
                  ...
                  if (!R && sid && code) {
                      ...
                      if ([
                          LoginType.ms,
                          LoginType.fs
                      ].includes(type)) verifier = $.store.get('codeVerifier') // ms/fs PKCE 需要
                      ;
     ...
     */ checkToken(app, token) {
          ...
          return _async_to_generator(function*() {
              ...
              try {
                  ...
                  const rs = yield $.get(`${cfg[cfg.mode].api}/${_.opt.api.checkToken}`, `token=${token}`);
     ...
     */ showHtml(p, v, lastHash) {
          ...
          ;
          if (p.dom !== v) p.dom = v;
          if (((_p_$el = p.$el) == null ? void 0 : _p_$el.dom) !== v) p.$el = $(v, true) // 加载name
          ;
     ...
     */ this.getCurrentPage = ()=>$.qu(`.${this.opt.showClass}`);
          ...
          // 当前 hash
          let hash = getHash();
          /** @type {*} */ let param = $.urlParam() // hash 前 或 后的参数，已使用 decodeURIComponent 解码
          ;
